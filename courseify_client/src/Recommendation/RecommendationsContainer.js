...
class RecommendationsContainer extends Component {
    ...
    getUserInfo() {
        ...
        const url = username != this.state.current_user.username ? `${process.env.REACT_APP_API_URL}/api/v1/users/${username}` 
    ...
    // refreshUserInfo() {
    //     const url = this.getMatch() ? `${process.env.REACT_APP_API_URL}/api/v1/users/` + this.getMatch().params.id : 
    //                                   `${process.env.REACT_APP_API_URL}/api/v1/profile`;
    //     setTimeout(() => {
    //         axios.get(url)
    //         .then(res => {
